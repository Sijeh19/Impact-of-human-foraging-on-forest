...
setwd("directory where file is located")
...
#-------------------------------------------------------------------------------
### 3.2	DIAMETER SIZE DISTRIBUTION
#-------------------------------------------------------------------------------
...
dbh_densityplot =ggplot(DBH_new, aes(x=log(DBH), color=Species, group = Species),
  ...
  annotate(geom="text", x=4.35, y=0.12, label="n", color="red", cex=6)+
  annotate(geom="text", x=3.78, y=0.09, label="m", color="red", cex=6)+
  annotate(geom="text", x=5.2, y=0.62, label="m & n = disturbance \n      in DBH", color="red", cex=5)
...
#-------------------------------------------------------------------------------
### 3.3	TREE STAND AND BASAL AREA DENSITIES ALONG THE ELEVATIONAL BAND
#-------------------------------------------------------------------------------
...
plotdensity <- ggplot(BA_Den, aes(x=Elevation, y=Density, colour = Category))+  
  ...
  theme(legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        text=element_text(size=18,  family="Times New Roman"))+
  annotate(geom="text", x=220, y=460, 
...
xl <- expression(Basal ~ Area ~ (m^2 ~ per ~ha))

plotBA<- ggplot(BA_Den, aes(x=Elevation, y=BA, colour = Category))+ 
  ...
  theme(legend.position = c(.9, .8),
        ...
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        text=element_text(size=18,  family="Times New Roman"))+
  annotate(geom="text", x=220, y=44, 
           label=expression("Edible: " *R^2*" = 0.19 ("*italic('p value')*" = 0.11)"),
           color="black", cex=5)+
  annotate(geom="text", x=224, y=42, 
           label=expression("Inedible: " *R^2*" = 0.00 ("*italic('p value')*" = 0.91)"),
           color="black", cex=5)
...
summary(lm(BA~Elevation, edible_BA))
summary(lm(BA~Elevation, inedible_BA))


#-------------------------------------------------------------------------------
### 3.4	SPECIES ABUNDANCE DISTRIBUTIONS AND RANK ABUNDANCE DISTRIBUTIONS
#-------------------------------------------------------------------------------
...
##plot your data. X-axis is abundance rank 
#(rank 1 is most abundant species, rank 2 second most abundant and so on)

### generate ranked species abundance for the three categories
ed.rad <-rad(ab2)
ined.rad <- rad(ab3)
all.rad <- rad(ab)

# generate the different models for total forest, edible and inedible 
## species for comparison

##Total forest
all.gs <- fitrad(ab, "gs")
all.ls <- fitsad(ab, "ls")
all.pl <- fitsad(ab, "poilog")
all.ln <- fitsad(ab, "ln", trunc=0.5)

all.gs.rad <- radpred(all.gs)
all.ls.rad <- radpred(all.ls)
all.pl.rad <- radpred(all.pl)
all.ln.rad <- radpred(all.ln)

## edible species
ed.gs <- fitrad(ab2, "gs")
ed.ls <- fitsad(ab2, "ls")
ed.pl <- fitsad(ab2, "poilog")
ed.ln <- fitsad(ab2, "ln", trunc=0.5)

ed.gs.rad <- radpred(ed.gs)
ed.ls.rad <- radpred(ed.ls)
ed.pl.rad <- radpred(ed.pl)
ed.ln.rad <- radpred(ed.ln)

## inedible species
ined.gs <- fitrad(ab3, "gs")
ined.ls <- fitsad(ab3, "ls")
ined.pl <- fitsad(ab3, "poilog")
ined.ln <- fitsad(ab3, "ln", trunc=0.5)

ined.gs.rad <- radpred(ined.gs)
ined.ls.rad <- radpred(ined.ls)
ined.pl.rad <- radpred(ined.pl)
ined.ln.rad <- radpred(ined.ln)


## computing the Akaike information criterion (AIC) for proposed models 
AICtab(ed.gs, ed.ls, ed.pl, ed.ln, base=TRUE)
AICtab(ined.gs, ined.ls, ined.pl, ined.ln, base=TRUE)
AICtab(all.gs, all.ls, all.pl, all.ln, base=TRUE)
...
## vitualize the SADS and RADs
windowsFonts(A = windowsFont("Times New Roman"))

par(mfrow = c(1, 2))

{plot(all.rad, ylim=c(1,60), xlim=c(1, 145), pch = 20, col = "brown",
      family = "A", cex.lab=1.5, cex.main = 1.4, cex.axis = 1.4)
  lines(all.pl.rad, col="blue", lwd=1.5)
  lines(all.ls.rad, col="red", lwd=1.5)
  lines(all.ln.rad, col="cyan", lwd=1.5)
  points(ined.rad, pch = 20,col = "black")
  lines(ined.pl.rad, col="blue", lwd=1.5)
  lines(ined.ls.rad, col="red", lwd=1.5)
  lines(ined.ln.rad, col="cyan", lwd=1.5)
  points(ed.rad, pch = 20,col = "dark green")
  lines(ed.pl.rad, col="blue", lwd=1.5)
  lines(ed.ls.rad, col="red", lwd=1.5)
  lines(ed.ln.rad, col="cyan", lwd=1.5)
  mtext('(a)', side=3, line = 0.5, at=-4,col = "black", cex=1.5)
  legend("topright", c("Logseries", "Poisson lognormal", 
                       "Truncated lognormal","Total forest", "Edible Category", "Inedbible Category"), lty=1, 
         col=c("red", "blue", "cyan","brown", "dark green", "black"),
         pch=c(NA, NA, NA,19, 19, 19), lwd=c(3,3,3,NA,NA,NA))}



{barplot(f, ylim=c(0, 40), xlab = "Abundance Octaves", family = "A", cex.lab=1.5,
          cex.main = 1.3, cex.axis = 1.4, col="brown")

barplot(f3, ylim=c(0, 32), family = "A", cex.lab=1.5,
          cex.main = 1.3, cex.axis = 1.4, col="black", add=T)

barplot(f2, ylim=c(0, 31), family = "A", cex.lab=1.5,
          cex.main = 1.3, cex.axis = 1.4, col = "dark green", add=T)
  
  points(f, pch = 16, col="red")
  points(f2, pch = 16, col="red")
  points(f3, pch = 16, col="red")
  lines(f, pch = 10, col="red")
  lines(f2, pch = 10, col="red")
  lines(f3, pch = 10, col="red")
  ...
  legend("topright",
         c("Gambin model",expression("Total("*alpha*"=3.3, CI:2.4-4.4)"), 
           ...
           lty=c(1, 0, 0, 0), lwd =c(3,3, 2.5,3), col=c("red","brown","black","dark green"), pch = c(16,15,15,15))
  }
...
#-------------------------------------------------------------------------------
### 3.5	THE TREND IN BETA DIVERSITY WITH INCREASING ELEVATION
#-------------------------------------------------------------------------------
